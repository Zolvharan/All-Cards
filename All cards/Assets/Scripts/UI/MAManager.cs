...
public class MAManager : MonoBehaviour
{
    ...
    void InitAbilityForm(bool isEditing, AbilityData currAbility = null)
    {
        ...
        mainManager.DisplayAbilityForm();
    }
    ...
    void ReOpenCreationForm(bool saveChanges)
    {
        if (saveChanges)
        {
            AbilityData newAbility = new AbilityData(nameField.text, directed, biased, ally, potencies, durations, costPotencies, costDurations, globalStats);
    ...
    {
        // Load data
        AbilityData newAbility = loadAbility;
    ...
    public void SaveAbility()
    {
        ...
        mainManager.DisplaySavedAbilityData(newAbility, isInChar, !isInChar && editing ? characterManager.lists[1].value : -1);
    }
    // Opens ability saving form in load
    public void LoadAbility()
    {
        mainManager.DisplaySavedAbilityData(null, isInChar);
    }
    ...
    public void IncrementStat(int statIndex)
    {
        ...
        if (statIndex >= 10 && globalStats[statIndex - 10] < MAX_STAT_NUMS[statIndex])
    ...
    public void DecrementStat(int statIndex)
    {
        ...
        if (statIndex >= 10 && globalStats[statIndex - 10] > MIN_STAT_NUMS[statIndex])
            statTextNums[statIndex].text = (--globalStats[statIndex - 10]).ToString();
        else if (statIndex < 10)
        {
            // Effect potency
    ...
    void SetViewNums()
    {
        ...
        energyElements.SetActive(effectView && potencyView ? true : false);
